...
def _connect(config_path: str = "config.json"):
    ...
    try:
        ...
        print(f"{LOG_PREFIX} MySQL 连接失败: {exc}")
        return None
...
def upsert_route_and_waypoints(
    ...
    try:
        ...
        print(f"{LOG_PREFIX} upsert_route_and_waypoints 失败: {exc}")
        return False
    finally:
        conn.close()


def get_route_progress(route_id: str, config_path: str = "config.json") -> Optional[Tuple[int, str]]:
    ...
    try:
        ...
        print(f"{LOG_PREFIX} get_route_progress 失败: {exc}")
        return None
    finally:
        conn.close()


def set_route_progress(route_id: str, next_wp_index: int, config_path: str = "config.json") -> bool:
    if not route_id:
        return False
    conn = _connect(config_path)
    if not conn:
    ...
    try:
        ...
        print(f"{LOG_PREFIX} set_route_progress 失败: {exc}")
        return False
    finally:
        conn.close()


def set_route_status(route_id: str, status: str, config_path: str = "config.json") -> bool:
    ...
    try:
        ...
        print(f"{LOG_PREFIX} set_route_status 失败: {exc}")
        return False
    finally:
        conn.close()
...
def _write_active_route_meta_file(meta: Dict[str, Any]) -> None:
    directory = os.path.dirname(ACTIVE_ROUTE_META_FILE)
    ...
    try:
        ...
        print(f"{LOG_PREFIX} 写入 {ACTIVE_ROUTE_META_FILE} 失败: {exc}")
...
def clear_active_route_meta() -> None:
    try:
        ...
        print(f"{LOG_PREFIX} 删除 {ACTIVE_ROUTE_META_FILE} 失败: {exc}")
    ...
    if isinstance(data[0], dict):
        for i, point in enumerate(data):
            ...
            rows.append(
                {
                    "seq": i + 1,
                    "name": str(point.get("name", str(i + 1))),
                    "lon": lon,
                    "lat": lat,
                    "action_type": int(point.get("action_type", point.get("actionType", 0))),
