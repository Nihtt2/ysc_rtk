...
"""
Unitree ZED - Camera extrinsic calibration
...
def _build_cam_base_RT(params: dict):
    ...
    返回：
      R_cam_base : 3×3，把 base_link 坐标系向量转到相机坐标系
      t_cam_base : 3×1，base_link 原点在相机坐标系中的坐标
    """
    tx        = float(params["tx"])
    ty        = float(params["ty"])
    tz        = float(params["tz"])
    ...
    # 零偏角时 base_link(X=前,Y=左,Z=上) → ZED cam(X=右,Y=前,Z=上) 的固定轴映射
    ...
    # t_cam_base = -R_cam_base @ t_base（相机在 base 下的安装位置取反）
    t_base     = [tx, ty, tz]
...
class ZedExtrinsic:
    ...
    def __init__(self, params: dict):
        ...
        # 公开输出（ZED 世界坐标系，X=右 Y=前 Z=上）
        self.base_tx         = None   # base_link x 位置，米
        self.base_ty         = None   # base_link y 位置，米
        self.base_heading_cw = None   # base_link 航向，顺时针为正，度
        self.base_forward_xy = None   # base_link 前向在世界 XY 的分量 (fx, fy)
        self.base_left_xy    = None   # base_link 左向在世界 XY 的分量 (lx, ly)
